...
$idp= base64_decode($_GET['idpers']);
$dtr= base64_decode($_GET['dateres']);
   ...
   function send_mail($to,$date_deb,$date_fin,$salle)
{
    ...
    @$body .= "<b style='color: #407DBF'>Reservation efféctué</b><br><br>.<p></p>la salle ".$salle." de ".$date_deb." jusqu'a ".$date_fin;
...
$a = mysql_query("UPDATE reserver set date_val=now() where id_pers='$idp' AND id_act='$ida' AND date_res='$dtr' ") or die(mysql_error());
if($a){
   send_mail($mail,$dtd,$dtf,$intitule);
   echo'<script>swal("Réservation validée", "", "success")</script>';
}
